...
CHECK_PACKR := $(shell command -v packr2 2> /dev/null)
...
check-packr:
ifndef CHECK_PACKR
	$(error "Packr2 is not installed. Please install packr2 and retry.")
endif
...
generate-migrations: check-go check-packr
...
generate-migrations: ## Generate migration code
	cd db && packr2
